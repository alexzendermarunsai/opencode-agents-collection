...
A hybrid OpenCode agent template based on the lean `a-team` workflow, with GPT-5.5 orchestration and Chinese-model specialist execution.

Use `american-chinese-team` when you want the same 11-agent product-delivery roster as `a-team`, a GPT-5.5-optimized orchestrator, and specialist model routing split across DeepSeek and GLM through the `opencode-go` provider.

## How It Differs From `chinese-team`

- Same lean 11-agent roster as `chinese-team` and `a-team`
- GPT-5.5 coordinates orchestration using the `a-team-gpt-5.5` orchestrator prompt
...
Choose this pack when the lean `a-team` workflow fits, but you want GPT-5.5 to coordinate Chinese-model specialists.

## Model Routing

| Work lane | Agents | Model |
| --- | --- | --- |
| Orchestration | `Agents Orchestrator` | `openai/gpt-5.5` |
...
- `agents/agents-orchestrator.md` - coordinates multi-agent delivery and routes work to the right specialists using GPT-5.5 orchestration guidance
...
8. `api-tester` when API risk exists
9. `reality-checker`
10. `technical-writer` when docs need to change

The GPT-5.5 orchestrator should preserve lean quality gates while routing specialist work according to the model table above.
...
- This pack assumes access to both `openai/gpt-5.5` and the `opencode-go` provider models shown above.
